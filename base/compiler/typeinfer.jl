...
function adjust_effects(sv::InferenceState)
    ...
    if is_consistent_ifnotreturned(ipo_effects) && is_consistent_argtype(rt)
        # in a case when the :consistent-cy here is only tainted by mutable allocations
        # (indicated by `CONSISTENT_IFNOTRETURNED`), we may be able to refine it if the return
        # type guarantees that the allocations are never returned
        consistent = ipo_effects.consistent & ~CONSISTENT_IFNOTRETURNED
        ipo_effects = Effects(ipo_effects; consistent)
    end
    if is_consistent_ifnoglobal(ipo_effects)
        if is_noglobal(ipo_effects)
            if all(i::Int->is_effect_free_argtype(sv.slottypes[i]), 1:narguments(sv))
                consistent = ipo_effects.consistent & ~(CONSISTENT_IFNOGLOBAL)
                ipo_effects = Effects(ipo_effects; consistent)
            end
    ...
    if isa(def, Method)
        ...
        if is_effect_overridden(override, :noglobal)
            ipo_effects = Effects(ipo_effects; noglobal=true)
        end
    end
